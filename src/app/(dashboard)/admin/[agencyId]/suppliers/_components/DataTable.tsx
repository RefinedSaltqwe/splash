...
const extractRowIds = (rows: number[], users: Supplier[]) => {
  const data = users;
...
export function DataTable<TData, TValue>({
      ...
      toast.success(
        `${
          modalIds?.length && modalIds?.length > 1
            ? `${data.count} suppliers have`
            : `${data.count} supplier has`
        } been deleted.`,
      );
  ...
  const deleteSelecetedUsers = () => {
    const getSupplier = extractRowIds(extractTableIndex(rowSelection), users);
    suppliersModal.onOpen(getSupplier, "supplier");
  };
