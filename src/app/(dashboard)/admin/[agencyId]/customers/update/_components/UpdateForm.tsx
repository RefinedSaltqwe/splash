...
import { UpdateCustomer } from "@/server/actions/update-customer/schema";
...
const UpdateForm: React.FC<UpdateFormProps> = ({ cid }) => {
  const router = useRouter();
  const queryClient = useQueryClient();
  const { data } = useQuery({
    queryKey: ["customer-", cid],
    queryFn: () => getCustomer(cid),
  });
  const { execute, isLoading } = useAction(updateCustomer, {
    onSuccess: (data) => {
  ...
  const form = useForm<z.infer<typeof UpdateCustomer>>({
  ...
  function onSubmit(values: z.infer<typeof UpdateCustomer>) {
    void execute({
      id: cid,
      name: values.name,
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="w-full">
        ...
        <div className="mt-6 flex items-center justify-end gap-4">
          <Button
            ...
            onClick={() => router.push(`/admin/${data!.agencyId}/customers`)}
          >
            Cancel
          </Button>
