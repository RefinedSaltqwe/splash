...
import { deleteInventoryItems } from "@/server/actions/delete-inventory-items";
...
const extractRowIds = (
  rows: number[],
  users: InventoryListBySubaccountIdAndSupplierMaterialsUsed[],
) => {
  const data = users;
...
export function DataTable<TData, TValue>({
  ...
  const inventoryItemData = useCurrentUserStore(
    (state) => state.inventoryItemData,
  );

  const { execute, isLoading } = useAction(deleteInventoryItems, {
    onSuccess: () => {
      //Filter all deleted items out
      setInventoryItemsData(
        inventoryItemData.filter((item) => !modalIds?.includes(item.id)),
      );
      toast.success(
        `${
          modalIds?.length && modalIds?.length > 1 ? "Items have" : "Item has"
        } been deleted.`,
      );
  ...
  const deleteSelecetedUsers = () => {
    const getItems = extractRowIds(
      extractTableIndex(rowSelection),
      inventoryList,
    );
    inventoryModal.onOpen(getItems, "inventory");
  };
