...
import PipelineValue from "@/components/shared/PipelineValue";
import SubaccountFunnelChart from "@/components/shared/SubaccountFunnelChart";
...
const SubaccountPageId = async ({
  ...
  await queryClient.prefetchQuery({
    ...
    queryFn: () => getPipelines(params.subaccountId),
  });
  ...
  const funnels: FunnelsWithFunnelPages[] = await db.funnel.findMany({
    where: {
  ...
  const funnelPerformanceMetrics: FunnelsWithFunnelPagesAndTotalFunnelVisits[] =
    funnels.map((funnel) => ({
      ...funnel,
      FunnelPages: funnel?.FunnelPages ?? [],
      id: funnel?.id ?? "",
      name: funnel?.name ?? "",
      createdAt: funnel?.createdAt ?? new Date(),
      updatedAt: funnel?.updatedAt ?? new Date(),
      ...
      liveProducts: funnel?.liveProducts ?? null,
